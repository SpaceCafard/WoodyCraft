...
<div class="container">
    ...
    <center>
        <form class="form1" action="{{ route('user.updateId', $user->id) }}" method="POST">
            @csrf
            <span class="signup">Modifier Mot de passe</span>
            @if(session()->has('info1'))
                <span class="text-danger">{{ session('info1') }}</span>
            @endif
            <input type="password" placeholder="Ancien Mot de passe" name="oldPwd" class="form--input">
            @if ($errors->has('password'))
                <span class="text-danger">{{ $errors->first('password') }}</span>
            @endif
            <input type="password" placeholder="Nouveau Mot de passe" name="verifPwd" class="form--input">
            @if(session()->has('info2'))
                <span class="text-danger">{{ session('info2') }}</span>
            @endif
            <input type="password" placeholder="Confirmer Mot de passe" name="password" class="form--input">
