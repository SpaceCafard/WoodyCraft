...
<div class="container">
    ...
    <center>
        <form action="{{ route('products.store') }}" enctype="multipart/form-data" method="POST" class="form">
            @csrf
            <span class="signup">Ajouter un produit</span>
            <div class="select">
            ...
            @endif
            <input type="text" placeholder="Nom du Produit" name="nameP" class="form--input">
            @if ($errors->has('description'))
                <span class="text-danger">Vous devez entrer une description</span>
            @endif
            <textarea placeholder="Description du Produit" name="description" rows="10" cols="100" class="form--input"></textarea>
            @if ($errors->has('price'))
                <span class="text-danger">Vous devez entrer un prix</span>
            @endif
            <input type="text" placeholder="Prix du Produit" name="price"  class="form--input">
            @if ($errors->has('stock'))
                <span class="text-danger">Vous devez indiquer le stocke disponible</span>
            @endif
            <input type="text" placeholder="Stocke Disponible" name="stock" class="form--input">
